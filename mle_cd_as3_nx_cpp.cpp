...
double plogl(const double& par_j, const vec& para, const vec& cmats, const double& cmat_d, const int& i, const int& j, const int& n, const int& t, const vec& y, const double& sigma2, const double& penalty){
  vec new_para = para;
  new_para((n-1)*i + j) = par_j;
  const sp_mat wmat = make_w_cpp(n, new_para);
  const sp_mat temp_mat = speye(n, n) - wmat;
  const vec e = kron(speye(t,t), temp_mat)*y;
...
sp_mat main_loop(const vec& y, const int& n, const int& t, const double& penalty, const double& th_tol, const char& det_mode, const bool& full_mode, double& sigma2, vec& temp_par, vec& new_par){
  sigma2 /= 2;
  for (int i=0; i < n; ++i){
    ...
    if (!temp_par(ind_in).is_zero()|| full_mode){
      ...
      for (int j = 0; j< n-1; ++j){
        if (temp_par((n-1)*i+j) != 0|| full_mode){
          ...
          const double temp_ye = sum(dot(temp_yjt, temp_wjm*y));
          ...
          if (std::abs(temp_a) < datum::eps){
            ...
            if (r_c >0 && r_c < lim && r_c > -lim){
              const double new_llik = plogl(r_c, new_par, cmats, cmat_d, i, j, n, t, y, sigma2*2, penalty);
  ...
  sigma2 = dot(temp_mat*y, temp_mat*y)/(n*t);
  return new_w;
}


// [[Rcpp::export]]
Rcpp::List mle_cd_nx_as3_cpp(const vec& y, const int& n, const int& t, const double& penalty, const double& th_tol, const char& det_mode,  const int& max_full_it){
  //initial sigma2
  double sigma2 = dot(y, y)/(n*t);
  //temp_variables
  sp_mat temp_w(speye(n,n));
  vec temp_par(n*(n-1), fill::zeros);
  vec new_par(n*(n-1), fill::zeros);
  //first full cycle
  sp_mat new_w = main_loop(y, n, t, penalty, th_tol, det_mode, true, sigma2, temp_par, new_par);
  //main cycle
  int full_count = 1;
  do{
    do{
      temp_w = new_w;
      new_w = main_loop(y, n, t, penalty, th_tol, det_mode, false, sigma2, temp_par, new_par);
    } while (!elem_0_check(temp_w, new_w));
    temp_w = new_w;
    new_w = main_loop(y, n, t, penalty, th_tol, det_mode, true, sigma2, temp_par, new_par);
    full_count += 1;
